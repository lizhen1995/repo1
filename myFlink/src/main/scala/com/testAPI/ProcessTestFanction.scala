...
/**
  * @Classname ProcessTestFanction
  * @Description TODO
  * @Date 2019/12/17 22:43
  * @Created by lz
  */
object ProcessTestFanction {
  def main(args: Array[String]): Unit = {
    ...
    val stream = env.socketTextStream("mydis", 7777)
    ...
    val dataStream = stream.map(data => {
      ...
      .assignTimestampsAndWatermarks( new BoundedOutOfOrdernessTimestampExtractor[SensorReading]( Time.seconds(1) ) {
        override def extractTimestamp(element: SensorReading): Long = element.time * 1000
      } )
...
// class F1 extends WindowFunction[SensorReading,SensorReading]{
//   override def apply(key: KEY, w: W, iterable: lang.Iterable[SensorReading], collector: Collector[SensorReading]): Unit = ???
// }
//class F2 extends ProcessWindowFunction[SensorReading,SensorReading]{
//  override def process(key: KEY, context: ProcessWindowFunction[SensorReading, SensorReading, KEY, W]#Context, iterable: lang.Iterable[SensorReading], collector: Collector[SensorReading]): Unit = ???
//}
//
class F3 extends ProcessFunction[SensorReading,SensorReading]{
