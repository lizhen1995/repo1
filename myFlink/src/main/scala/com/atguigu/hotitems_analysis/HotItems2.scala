...
object HotItems2 {
  def main(args: Array[String]): Unit = {
    ...
    val dataStream = env.readTextFile("file:///E:/myFlink/src/main/resources/UserBehavior.csv")
   // val dataStream = env.addSource( new FlinkKafkaConsumer011[String]("hotitems", new SimpleStringSchema(), properties) )
      .map( data => {
    ...
    val processedStream = dataStream
      ...
      .aggregate( new CountAgg1(), new WindowResult1() )   // 窗口聚合
      //.keyBy(_.windowEnd)    // 按照窗口分组
      .process( new TopNHotItems1(3) )
...
// 自定义预聚合函数计算平均数
class AverageAgg1() extends AggregateFunction[UserBehavior, (Long, Int), Double]{
...
class WindowResult1() extends RichWindowFunction[Long, ItemViewCount, Long, TimeWindow]{
  override def apply(key: Long, window: TimeWindow, input: Iterable[Long], out: Collector[ItemViewCount]): Unit = {
    out.collect( ItemViewCount(key, window.getEnd, input.iterator.next(),1) )
  }
 // override def apply(key: Long, window: TimeWindow, input: Iterable[Long], out: Collector[ItemViewCount]): Unit = {
...
// 自定义的处理函数
class TopNHotItems1(topSize: Int) extends ProcessFunction[ ItemViewCount, String] with CheckpointedFunction{
  ...
  override def processElement(value: ItemViewCount, ctx: ProcessFunction[ ItemViewCount, String]#Context, out: Collector[String]): Unit = {
    // 把每条数据存入状态列表
    itemState.add(value)
    // 注册一个定时器
